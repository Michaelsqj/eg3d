...
@persistence.persistent_class
class SuperresGenerator128(torch.nn.Module):
    def __init__(self, out_channels=32*3):
        super().__init__()
        # load 128x128 pretrained model
        path_128_pretrained = "imagenet64.pkl"
        ...
        assert self.mapping.num_ws ==  self.synthesis.num_ws
        ...
        # for name in self.synthesis.layer_names[:-1]:
        #     getattr(self.synthesis, name).requires_grad_(False)
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ...
        #     for name, w in zip(self.synthesis.layer_names[:-1], ws[1:-1]):
        #         x = getattr(self.synthesis, name)(x, w, **synthesis_kwargs)

        # for name, w in zip(self.synthesis.layer_names[-1:], ws[-1:]):
        #     x = getattr(self.synthesis, name)(x, w, **synthesis_kwargs)
